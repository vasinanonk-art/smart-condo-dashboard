...
DESIGN_CSS = (ROOT / "frontend/assets/dashboard_household_design_system.css").read_text()
...
def test_controls_are_two_column_equal_height_grid():
    compact = _compact(DESIGN_CSS)
    controls = compact.split(".household-action-grid{", 1)[1].split("}", 1)[0]
    assert "display:grid" in controls
    assert "grid-template-columns:repeat(2,minmax(120px,1fr))" in controls
    button = compact.split(".household-action-button{", 1)[1].split("}", 1)[0]
...
def test_warning_box_is_yellow_and_has_no_fixed_height():
    compact = _compact(DESIGN_CSS)
    warning = compact.split(".household-warning-box{", 1)[1].split("}", 1)[0]
...
def test_all_household_styles_are_namespaced():
    without_media = re.sub(r"@media[^{]+\\{", "", CSS + DESIGN_CSS)
...
def test_requested_viewport_widths_have_non_overlapping_column_plan():
    ...
    expected_columns = {1920: 4, 1600: 3, 1440: 3, 1280: 2, 1024: 2, 768: 1, 390: 1}
    for viewport, columns in expected_columns.items():
        ...
        if viewport <= 990:
            columns = 1
        minimum = 0 if viewport <= 520 else 320
        required = columns * minimum + (columns - 1) * 16
