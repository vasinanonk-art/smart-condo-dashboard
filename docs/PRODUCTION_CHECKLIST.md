# v1.0.3 Production Checklist

Verification date: **2026-08-10 Asia/Bangkok**

Release commit: the commit referenced by annotated tag `v1.0.3`
...
- [x] No destructive state or history migration is part of v1.0.3.
...
- [x] Create the local annotated `v1.0.3` tag after required repository checks.
...
foundations are included in the v1.0.3 tag candidate.
...
| Dark theme | Complete | v1.0.3 is explicitly dark-only |
...
| Git tag | Complete locally | Annotated `v1.0.3`; push pending |
...
git worktree add /opt/smart-condo-dashboard-v1.0.3 \
  v1.0.3
cd /opt/smart-condo-dashboard-v1.0.3
test -z "$(git status --short)" &&
test "$(git rev-parse HEAD)" = "$(git rev-list -n 1 v1.0.3)" &&
...
verified and a state rollback is necessary. v1.0.3 does not require a
...
```sh
git tag -a v1.0.3 \
  -m "Smart Condo Dashboard v1.0.3" \
  HEAD
git show --stat --oneline v1.0.3
