# v1.0.2 Production Checklist

Verification date: **2026-08-10 Asia/Bangkok**

Release commit: the commit referenced by annotated tag `v1.0.2`
...
- [x] No destructive state or history migration is part of v1.0.2.
...
- [x] Run `sudo ./install.sh --dry-run` against the tag candidate.
...
- [x] Create the local annotated `v1.0.2` tag after required repository checks.
...
foundations are included in the v1.0.2 tag candidate.
...
| Dark theme | Complete | v1.0.2 is explicitly dark-only |
...
| Git tag | Complete locally | Annotated `v1.0.2`; push pending |
...
git worktree add /opt/smart-condo-dashboard-v1.0.2 \
  v1.0.2
cd /opt/smart-condo-dashboard-v1.0.2
test -z "$(git status --short)" &&
test "$(git rev-parse HEAD)" = "$(git rev-list -n 1 v1.0.2)" &&
...
verified and a state rollback is necessary. v1.0.2 does not require a
...
```sh
git tag -a v1.0.2 \
  -m "Smart Condo Dashboard v1.0.2" \
  HEAD
git show --stat --oneline v1.0.2
