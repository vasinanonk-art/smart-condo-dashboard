(() => {
    ...
    try {
      ...
      if (payload?.tv) {
        ...
        S.tv.lastValid = {
          ...previous,
          ...payload.tv,
          ts: payload.tv.last_update_ts || previous.ts,
          last_update_ts: payload.tv.last_update_ts || previous.last_update_ts,
        };
      }
    } catch (error) {
  ...
  renderEntertainment = function renderSynchronizedEntertainment() {
    ...
    const statusItems = [
      ...
      ['Current App', tvValue(tv.app), ''],
      ['Input', tvValue(tv.input), ''],
      ['Volume', tvValue(tv.volume), ''],
      ['Mute', tv.mute === true ? 'Muted' : tv.mute === false ? 'Sound on' : tvValue(tv.mute), ''],
      ['Last Update', when(tv.last_update_ts || tv.ts), ''],
    ];
  ...
  function renderTopology() {
    ...
    if (!data) {
      graph.innerHTML = '<div class="empty">Topology data is not available.</div>';
      return;
    }
