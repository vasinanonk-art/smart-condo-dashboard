(() => {
  ...
  const topologyState = {data:null, selected:null, mode:null, viewBox:null, fitted:null, dragging:false, dragStart:null};
  ...
  const SITE = {
    internet:'cloud', cloudflare_wan:'cloud', condo_router:'condo', tinkerboard:'condo', dashboard:'condo',
    mqtt:'condo', presence:'condo', lg_tv:'condo', sonoff:'condo', camera:'condo', tuya:'condo',
    electricity:'condo', pm25:'condo', zerotier_condo:'condo', zerotier_tunnel:'cloud',
    zerotier_home:'home', truenas:'home', home_assistant:'home'
  };
  const SOURCE = {
    internet:'https', cloudflare_wan:'dns_and_https', condo_router:'icmp', tinkerboard:'local_runtime',
    dashboard:'local_runtime', mqtt:'mqtt', presence:'mqtt', lg_tv:'mqtt', sonoff:'sonoff_cloud',
    camera:'local_runtime', tuya:'home_assistant', electricity:'tuya_local', pm25:'home_assistant',
    zerotier_condo:'local_runtime', zerotier_tunnel:'zerotier', zerotier_home:'zerotier',
    truenas:'zerotier', home_assistant:'home_assistant'
  };
  ...
  window.nav = function topologyNav(page) {
    ...
    if (page === 'topology' && document.getElementById('pageTitle')) {
      ...
      requestAnimationFrame(() => renderTopology(true));
    }
  };
  ...
  window.renderEntertainment = function renderEntertainmentUnified() {
    ...
    const items = [['Status',status,online?'ok':bridgeOnly?'warn':'bad'],['Current App',tvValue(tvState.app),''],['Input',tvValue(tvState.input),''],['Volume',tvValue(tvState.volume),''],['Mute',tvState.mute===true?'Muted':tvState.mute===false?'Sound on':tvValue(tvState.mute),''],['Last Update',thailandTime(tvState.last_update_ts||tvState.last_heartbeat_ts),'']];
  ...
  function physicalSite(node) { return node?.physical_site || node?.metadata?.physical_site || SITE[node?.id] || 'unknown'; }
  function dataSource(node) { return node?.data_source || node?.diagnostics?.source || node?.metadata?.source || SOURCE[node?.id] || 'unknown'; }
  ...
  function renderDetail(node) {
    ...
    const primary = [
      ['Status',node.online==null?'Unknown':node.online?'Online':'Offline'],['Health',node.health||'unknown'],
      ['Physical Site',physicalSite(node)],['Data Source',dataSource(node)],
      ['Latency',node.latency_ms==null?'Unknown':`${node.latency_ms} ms`],['Last Update',thailandTime(node.last_update_ts)],
      ['Dependencies',(node.dependencies||[]).join(', ')||'None'],['Dependents',(node.dependents||[]).join(', ')||'None'],
  ...
  const ORDER = ['internet','cloudflare_wan','condo_router','tinkerboard','dashboard','mqtt','presence','lg_tv','sonoff','camera','tuya','pm25','electricity','zerotier_condo','zerotier_tunnel','zerotier_home','truenas','home_assistant'];
  const TRUNK = ['internet','cloudflare_wan','condo_router','tinkerboard'];
  const CONDO_RANKS = [['dashboard','mqtt'],['sonoff','camera','tuya'],['presence','lg_tv','pm25','electricity']];
  const REMOTE_RANKS = [['zerotier_condo'],['zerotier_tunnel'],['zerotier_home'],['truenas'],['home_assistant']];
  ...
  function placeRank(ids, centerX, y, nodeW, hGap, positions) {
    ...
    ids.forEach(id => { positions[id]=[Math.round(x),Math.round(y)]; x += nodeW+hGap; });
  }
  ...
  function buildLayout(nodes) {
    ...
    const positions = {};
    if (mode==='mobile') {
      const nodeW=Math.min(300,available-28), nodeH=54, vGap=34;
      let y=34;
      ORDER.filter(id=>present.has(id)).forEach(id=>{positions[id]=[Math.round((available-nodeW)/2),y];y+=nodeH+vGap;});
      return {mode,width:available,height:y+30,nodeW,nodeH,positions,groupPadding:18};
    }
    const nodeW=mode==='desktop'?154:140, nodeH=54, hGap=mode==='desktop'?52:38, vGap=52, pad=32;
    const width=Math.max(available, mode==='desktop'?1040:820);
    const trunkX=width/2;
    let y=30;
    TRUNK.filter(id=>present.has(id)).forEach(id=>{positions[id]=[Math.round(trunkX-nodeW/2),y];y+=nodeH+vGap;});
    const splitY=y+18;
    const condoCenter=width*0.32, remoteCenter=width*0.76;
    CONDO_RANKS.forEach((rank,index)=>placeRank(rank.filter(id=>present.has(id)),condoCenter,splitY+index*(nodeH+vGap),nodeW,hGap,positions));
    REMOTE_RANKS.forEach((rank,index)=>placeRank(rank.filter(id=>present.has(id)),remoteCenter,splitY+index*(nodeH+vGap),nodeW,hGap,positions));
    const maxBottom=Math.max(...Object.values(positions).map(([,py])=>py+nodeH));
    return {mode,width,height:maxBottom+pad+34,nodeW,nodeH,positions,groupPadding:pad};
  }
  ...
  function orthogonalPath(from,to,layout,index=0) {
    ...
    const elbowY=Math.max(y1+18,Math.min(y2-18,(y1+y2)/2+offset));
    return {d:`M${x1},${y1} L${x1},${elbowY} L${x2},${elbowY} L${x2},${y2}`,markX:(x1+x2)/2,markY:elbowY};
  }
  ...
  function groupBounds(ids,layout,label) {
    ...
    const x=Math.min(...points.map(([px])=>px))-pad, y=Math.min(...points.map(([,py])=>py))-pad;
    const right=Math.max(...points.map(([px])=>px+layout.nodeW))+pad, bottom=Math.max(...points.map(([,py])=>py+layout.nodeH))+pad;
  ...
  function fitToView(showFeedback=false) {
    ...
    if(!svg||!topologyState.fitted)return;
    topologyState.viewBox={...topologyState.fitted};
    svg.setAttribute('viewBox',`${topologyState.viewBox.x} ${topologyState.viewBox.y} ${topologyState.viewBox.w} ${topologyState.viewBox.h}`);
    svg.classList.add('fitting');
    setTimeout(()=>svg.classList.remove('fitting'),150);
    if(showFeedback){
      const button=document.getElementById('topologyFit');
      if(button){button.classList.add('active');button.textContent='Fitted';setTimeout(()=>{button.classList.remove('active');button.textContent='Fit to View';},700);}
      window.toast?.('Topology fitted to view');
    }
  }
  ...
  function bindPanZoom(svg) {
    svg.onwheel=event=>{
      ...
      const box=topologyState.viewBox||topologyState.fitted;if(!box)return;
      const factor=event.deltaY>0?1.12:0.88;
      const nextW=Math.max(box.w*0.55,Math.min(box.w*2.5,box.w*factor));
      const nextH=nextW*(box.h/box.w);
      const next={x:box.x+(box.w-nextW)/2,y:box.y+(box.h-nextH)/2,w:nextW,h:nextH};
      topologyState.viewBox=next;svg.setAttribute('viewBox',`${next.x} ${next.y} ${next.w} ${next.h}`);
    },{passive:false};
    svg.onpointerdown=event=>{topologyState.dragging=true;topologyState.dragStart={x:event.clientX,y:event.clientY,box:{...(topologyState.viewBox||topologyState.fitted)}};svg.setPointerCapture(event.pointerId);};
    svg.onpointermove=event=>{if(!topologyState.dragging||!topologyState.dragStart)return;const rect=svg.getBoundingClientRect(),start=topologyState.dragStart,dx=(event.clientX-start.x)*start.box.w/rect.width,dy=(event.clientY-start.y)*start.box.h/rect.height;const next={...start.box,x:start.box.x-dx,y:start.box.y-dy};topologyState.viewBox=next;svg.setAttribute('viewBox',`${next.x} ${next.y} ${next.w} ${next.h}`);};
    svg.onpointerup=()=>{topologyState.dragging=false;topologyState.dragStart=null;};
    svg.onpointercancel=svg.onpointerup;
  }

  function renderGraph(nodes,forceFit=false) {
    const host=document.getElementById('topologyGraph');if(!host)return;
    const layout=buildLayout(nodes), map=new Map(nodes.map(node=>[node.id,node])), edges=[];
    nodes.forEach(node=>(node.dependencies||[]).forEach(dep=>{if(map.has(dep))edges.push([dep,node.id]);}));
    const groups=[groupBounds(['internet','cloudflare_wan'],layout,'CLOUD'),groupBounds(['condo_router','tinkerboard','dashboard','mqtt','presence','lg_tv','sonoff','camera','tuya','pm25','electricity'],layout,'CONDO'),groupBounds(['zerotier_condo','zerotier_tunnel','zerotier_home'],layout,'ZEROTIER'),groupBounds(['truenas','home_assistant'],layout,'HOME')].join('');
    const edgeSvg=edges.map(([from,to],index)=>{const route=orthogonalPath(from,to,layout,index);if(!route)return'';const cls=edgeHealth(map.get(from),map.get(to));return `<path class="topology-edge ${cls}" d="${route.d}"/>${cls==='healthy'?`<path class="topology-edge-packet" d="${route.d}"/>`:''}${cls==='offline'?`<text class="topology-edge-x" x="${route.markX-5}" y="${route.markY+6}">×</text>`:''}`;}).join('');
    const nodeSvg=nodes.map(node=>{const pos=layout.positions[node.id];if(!pos)return'';const status=node.online===true?'Online':node.online===false?'Offline':node.health==='warning'?'Warning':'Unknown';const selected=node.id===topologyState.selected?' selected':'';return `<g class="topology-node-svg ${healthClass(node.health)}${selected}" data-topology-node="${safe(node.id)}" tabindex="0" transform="translate(${pos[0]} ${pos[1]})"><rect class="topology-node-bg" width="${layout.nodeW}" height="${layout.nodeH}"/><circle class="topology-node-dot" cx="17" cy="17" r="6"/><text class="topology-node-name" x="30" y="20">${safe(node.name)}</text><text class="topology-node-status" x="17" y="40">${safe(status)}${node.latency_ms!=null?` · ${safe(node.latency_ms)} ms`:''}</text></g>`;}).join('');
    const padding=24;
    topologyState.fitted={x:-padding,y:-padding,w:layout.width+padding*2,h:layout.height+padding*2};
    if(forceFit||!topologyState.viewBox)topologyState.viewBox={...topologyState.fitted};
    host.innerHTML=`<svg class="topology-svg" viewBox="${topologyState.viewBox.x} ${topologyState.viewBox.y} ${topologyState.viewBox.w} ${topologyState.viewBox.h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Smart condo dependency topology">${groups}${edgeSvg}${nodeSvg}</svg>`;
    const svg=host.querySelector('svg');bindPanZoom(svg);
    const fit=document.getElementById('topologyFit');if(fit)fit.disabled=false;
    host.querySelectorAll('[data-topology-node]').forEach(element=>{const select=()=>{topologyState.selected=element.dataset.topologyNode;renderDetail(nodeById(topologyState.selected));renderGraph(topologyState.data?.nodes||[],false);};element.onclick=select;element.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();select();}};});
  }

  function renderTopology(forceFit=false) {
    const data=topologyState.data,roots=document.getElementById('topologyRoots'),events=document.getElementById('topologyEvents'),score=document.getElementById('topologyHealth');
  ...
  let resizeTimer;
  window.addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{if(window.currentPage()==='topology'){topologyState.viewBox=null;renderTopology(true);}},140);},{passive:true});
