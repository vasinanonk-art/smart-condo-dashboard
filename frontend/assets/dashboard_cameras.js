(() => {
  ...
  function cameraCard(camera) {
    ...
    const snapshot = capabilities.snapshot && camera.online === true
      ...
      : '<div class="camera-snapshot-unavailable">Snapshot unavailable</div>';
    const actions = [
  ...
  async function render() {
    ...
    try {
      ...
      host.innerHTML = `<section class="camera-page-head"><div><h2>Cameras</h2><p>View current camera availability and open a live view.</p></div><span class="muted">${cameras.length} camera${cameras.length === 1 ? '' : 's'}</span></section>${cameras.length ? `<div class="camera-grid">${cameras.map(cameraCard).join('')}</div>` : '<div class="card camera-empty">No camera configuration is available.</div>'}`;
