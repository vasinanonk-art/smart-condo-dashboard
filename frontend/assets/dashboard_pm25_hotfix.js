(() => {
  ...
  function selectSampleIndex(pointerX,positions) {
    ...
    let low=0,high=last;
    while(low+1<high){const mid=Math.floor((low+high)/2);if(positions[mid]<=pointerX)low=mid;else high=mid;}
  ...
  function hide(id){const svg=document.getElementById(id),layer=svg?.querySelector('.hover-layer'),tooltip=svg?.parentElement?.querySelector('.tooltip'),debug=svg?.parentElement?.querySelector('.chart-debug-geometry');if(layer)layer.style.display='none';if(tooltip)tooltip.style.display='none';if(debug)debug.style.display='none';state.delete(id);}
  ...
  function install(id,rows,series){
    ...
    line.setAttribute('y1',plot.top);line.setAttribute('y2',plot.bottom);
    hit.setAttribute('x',plot.left);hit.setAttribute('y',plot.top);hit.setAttribute('width',plot.right-plot.left);hit.setAttribute('height',plot.bottom-plot.top);hit.style.pointerEvents='all';hit.style.fill='transparent';hit.style.cursor='crosshair';
    // Keep the interaction surface above lines, points and crosshair at all times.
    svg.appendChild(hit);

    const move=event=>{
      const pointer=event.touches?.[0]||event.changedTouches?.[0]||event,svgPoint=clientToSvg(svg,pointer.clientX,pointer.clientY);if(!svgPoint)return;
      // The hit rectangle is the authoritative in-plot boundary. Small browser/SVG
      // rounding outside the mathematical edge is clamped instead of clearing state.
      ...
      const index=selectSampleIndex(pointerX,positions);if(index<0)return;
      const row=valid[index],sampleX=positions[index];
      line.setAttribute('x1',sampleX);line.setAttribute('x2',sampleX);points.innerHTML='';
      ...
      const wrapRect=wrap.getBoundingClientRect(),anchor=svgToClient(svg,sampleX,pointerY);if(anchor){const localX=anchor.x-wrapRect.left,localY=anchor.y-wrapRect.top,tipW=tooltip.offsetWidth||180,tipH=tooltip.offsetHeight||70,gap=12,pad=8;let left=localX+gap;if(left+tipW>wrapRect.width-pad)left=localX-tipW-gap;left=Math.max(pad,Math.min(Math.max(pad,wrapRect.width-tipW-pad),left));let top=localY-tipH/2;top=Math.max(pad,Math.min(Math.max(pad,wrapRect.height-tipH-pad),top));tooltip.style.left=`${left}px`;tooltip.style.top=`${top}px`;}
      state.set(id,{index,pointerX,sampleX,count:valid.length});
      if(DEBUG){let debug=wrap.querySelector('.chart-debug-geometry');if(!debug){debug=document.createElement('div');debug.className='chart-debug-geometry';wrap.appendChild(debug);}debug.style.display='block';debug.textContent=`plot ${plot.left.toFixed(1)},${plot.top.toFixed(1)} → ${plot.right.toFixed(1)},${plot.bottom.toFixed(1)} · pointerX ${pointerX.toFixed(1)} · index ${index} · sampleX ${sampleX.toFixed(1)} · visible ${valid.length}`;}
    };

    hit.onmousemove=move;hit.onpointermove=move;hit.onpointerenter=move;hit.onpointerdown=move;
    hit.ontouchstart=event=>{event.preventDefault();move(event);};hit.ontouchmove=event=>{event.preventDefault();move(event);};
    hit.onmouseleave=()=>hide(id);hit.onpointerleave=()=>hide(id);hit.ontouchcancel=()=>hide(id);
  }
  ...
  window.DashboardChartInteraction=Object.freeze({selectSampleIndex,samplePositions,visibleRowsFor});
})();
