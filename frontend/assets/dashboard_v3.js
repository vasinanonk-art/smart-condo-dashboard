...
function renderSystem() {
  ...
  $('systemDetails').innerHTML = `<div class="system-overall ${required.length ? 'warning' : 'healthy'}"><span>Overall Health</span><strong>${overall}</strong><small>${required.length ? `${required.length} item${required.length === 1 ? '' : 's'} need attention` : 'No action is required right now.'}</small></div><section class="system-action-required"><h3>Action Required</h3>${required.length ? `<ul>${required.map(item => `<li>${safeText(item)}</li>`).join('')}</ul>` : '<p class="system-clear">No action required.</p>'}</section><section class="system-incidents"><h3>Recent Incidents</h3>${incidents.length ? `<ul>${incidents.map(item => `<li>${safeText(item)}</li>`).join('')}</ul>` : '<p class="system-clear">No recent incidents.</p>'}</section><section class="system-service-summary"><h3>Services</h3><div class="system-summary-grid"><div><span>Dashboard</span><strong class="ok">${safeText(data.service || 'Unknown')}</strong></div><div><span>MQTT</span><strong class="${data.mqtt?.connected ? 'ok' : 'bad'}">${data.mqtt?.connected ? 'Online' : 'Offline'}</strong></div><div><span>Lighting</span><strong class="${S.sonoffAvailable ? 'ok' : 'bad'}">${S.sonoffAvailable ? 'Online' : 'Unavailable'}</strong></div><div><span>Cameras</span><strong class="${data.camera?.config_loaded === false ? 'warn' : 'ok'}">${data.camera?.config_loaded === false ? 'Unavailable' : `${data.camera?.count || 0} configured`}</strong></div></div></section><section class="system-storage-summary"><h3>Storage / Backup</h3><div class="system-summary-grid"><div><span>History</span><strong class="ok">${history.history_store_path ? 'Available' : 'Not available'}</strong></div><div><span>Backup</span><strong class="muted">Not available</strong></div></div></section><details class="system-technical"><summary>Advanced technical details</summary><div class="kv"><span>Application version</span><strong>${safeText(data.version || '-')}</strong></div><div class="kv"><span>Sonoff Cloud</span><strong class="${cloud.cls}">${cloud.label}</strong></div><div class="kv"><span>Sonoff devices</span><strong>${(S.sonoff.devices || []).length}</strong></div><div class="kv"><span>Sonoff last successful sync</span><strong>${when(S.sonoffLastSyncTs)}</strong></div>${safeError ? `<div class="kv"><span>Sonoff error</span><strong class="bad">${safeText(safeError)}</strong></div>` : ''}<div class="kv"><span>Home Assistant PM2.5</span><strong class="${data.home_assistant?.configured ? 'ok' : 'warn'}">${data.home_assistant?.configured ? 'Configured' : 'Unavailable'}</strong></div><div class="kv"><span>History store</span><strong>${safeText(history.history_store_path || '-')}</strong></div><div class="kv"><span>History loaded/appended/pruned</span><strong>${history.loaded_count || 0} / ${history.appended_count || 0} / ${history.pruned_count || 0}</strong></div><div class="kv"><span>Camera configuration</span><strong>${data.camera?.config_loaded ? 'Loaded' : 'Unavailable'} · ${data.camera?.count || 0} cameras</strong></div></details>`;
}
