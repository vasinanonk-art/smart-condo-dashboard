(() => {
  ...
  function bind() {
    ...
    document.getElementById('runMaintenance')?.addEventListener('click', async () => { if (!confirm('Run maintenance now? This may prune or update persistent state.')) return; state.maintenance = await jsonRequest('/api/maintenance/run','POST',{}); render(); });
