"""Journal-backed, redacted audit records for camera command attempts."""
...
def emit(
    *,
    ...
    record = {
        "event": "camera_command_audit",
        "timestamp": int(timestamp if timestamp is not None else time.time()),
        "authenticated_user": _safe_user(user),
        "device": _safe_identifier(device_id, "invalid_device"),
        "command_type": _safe_identifier(command_type, "unknown"),
        "value": _safe_value(value),
        "outcome": _safe_identifier(outcome, "failed"),
        "http_status": int(http_status),
        "result_code": _safe_identifier(result_code, "unknown_result"),
    ...
    line = "camera_command_audit " + json.dumps(
        record,
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=True,
